...
use crate::pdf::document::normalize_internal_fit_r;
...
use crate::{
    context, unsafe_impl_ffi_wrapper, DestinationKind, Error, FFIWrapper, Matrix, Page, Rect,
};
...
impl PdfPage {
    ...
    ///   (see [`DestinationKind::encode_into`])
    ...
    pub fn add_links_with_inv_ctm(
        ...
        doc: &mut PdfDocument,
        links: &[PdfLink],
        annot_inv_ctm: impl FnOnce(&PdfObject) -> Result<Matrix, Error>,
        mut dest_inv_ctm: impl FnMut(&PdfObject) -> Result<Matrix, Error>,
    ) -> Result<(), Error> {
        if links.is_empty() {
            return Ok(());
        }
        ...
        let mut page_cache: HashMap<u32, (PdfObject, Matrix)> = HashMap::new();
            ...
            unsafe {
                ...
                let action = match LinkDestination::from_uri(&self.doc, uri) {
                    Ok(Some(dest)) => {
                        ...
                        let page = dest.loc.page_in_chapter;

                        let kind = normalize_internal_destination_kind(&self.doc, page, dest.kind);
                        let kind = match kind {
                            Ok(kind) => kind,
                    ...
                    Ok(None) => match parse_external_link(uri.to_string_lossy().as_ref()) {
                        Some(action) => match action {
                            PdfAction::GoTo(PdfDestination::Page { page, kind }) => {
                                let kind =
                                    normalize_internal_destination_kind(&self.doc, page, kind);
...
pub(super) fn normalize_internal_destination_kind(
    doc: &PdfDocument,
    page: u32,
    kind: DestinationKind,
) -> Result<DestinationKind, Error> {
    if !matches!(kind, DestinationKind::FitR { .. }) {
        return Ok(kind);
    }

    let page_obj = doc.find_page(page as i32)?;
    let ctm = page_obj.page_ctm()?;
    Ok(normalize_internal_fit_r(kind, &ctm))
}
