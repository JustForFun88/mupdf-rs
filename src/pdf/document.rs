...
mod test {
    ...
    fn test_as_string_indirect_delete() {
        ...
        // Use a large string so the allocation is more likely to be reused.
        ...
        // 6. Allocate new objects to try to overwrite the freed memory.
        let overwrite = "B".repeat(1000);
        for _ in 0..10 {
            let _ = PdfObject::new_string(&overwrite);
        }
    ...
    fn test_as_bytes_owned_indirect_delete() {
        ...
        // as_bytes() returns an owned PdfVec — safe even after deletion
