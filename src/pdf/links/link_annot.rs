...
///
/// Created by [`PdfPage::link_annots`](crate::pdf::PdfPage::link_annots) by iterating
/// the page's `/Annots` array and filtering for link annotations.
///
/// # Why not `PdfAnnotation`?
///
/// MuPDF's `pdf_sync_annots` (pdf-annot.c) skips `Subtype=Link` entries with an explicit
/// `continue`, so a [`PdfAnnotation`](crate::pdf::PdfAnnotation) of type `Link` can never be
/// produced by the annotation machinery. This type bypasses that by working directly with the
/// annotation dictionary object retrieved from the page's `/Annots` array.
///
...
impl PdfLinkAnnot {
    ...
    pub fn action(
        ...
        parse_link_action_from_annot_dict(&self.inner, doc, page_num)
    }
    ...
    pub fn set_action_with_resolver(
        ...
        set_link_action_on_annot_dict(doc, &mut self.inner, action, resolver)
    }
    ...
    pub fn to_pdf_link(
        ...
        let Some(action) = self.action(doc, page_num)? else {
            return Ok(None);
        };
