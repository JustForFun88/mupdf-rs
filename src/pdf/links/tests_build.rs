...
use super::*;
use crate::pdf::page::normalize_internal_destination_kind;
                ...
                let action = match parse_external_link(uri.to_string_lossy().as_ref()) {
                    Some(action) => match action {
                        PdfAction::GoTo(PdfDestination::Page { page, kind }) => {
                            let kind = normalize_internal_destination_kind(&doc, page, kind)?;
                            PdfAction::GoTo(PdfDestination::Page { page, kind })
                        }
...
fn test_fitr_goto_and_gotor_links() {
    ...
    assert_links_match(&pdf, &links).unwrap();
    // Fails because of https://bugs.ghostscript.com/show_bug.cgi?id=709082
    assert_raw_uri_matches(&pdf, &links).unwrap_err();
...
fn test_fitr_rotated_target_page() {
    ...
    for (target_page_idx, &rotation) in rotations.iter().enumerate() {
        ...
        for (org, raw) in links.iter().zip(raw_links.iter()) {
            if org.action.to_string() != *raw {
                all_match = false;
                break;
            }
        }
        ...
        assert!(
            !all_match,
            "{}: Expected Raw URI match to FAIL due to Ghostscript bug, but it SUCCEEDED.",
            context_msg
        );
    }
}
