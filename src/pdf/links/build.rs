...
pub(crate) fn build_link_annotation<F>(
    ...
    page_obj: &PdfObject,
    link: &PdfLink,
    annot_inv_ctm: &Matrix,
    fn_dest_inv_ctm: &mut F,
    page_cache: &mut HashMap<u32, (PdfObject, Matrix)>,
) -> Result<PdfObject, Error>
where
    F: FnMut(&PdfObject) -> Result<Matrix, Error>,
{
    let rect = link.bounds.transform(annot_inv_ctm);
    ...
    match &link.action {
        PdfAction::GoTo(dest) => match dest {
            PdfDestination::Page { page, kind } => {
                ...
                let dest_kind = kind.transform(dest_inv_ctm);
